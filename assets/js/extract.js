 ...
 * machine; only the extracted text is sent on for question writing.
 *
 * The output is a flat list of units, one per slide or page, because that is
 * what lets the reviewer cover a whole deck instead of its first few slides.
 */
(function (root) {
  'use strict';

  /* pdf.js is the one thing worth loading from elsewhere. Local copies in
     assets/vendor win when they are there, so a pinned offline deploy is
     possible; otherwise a CDN answers. */
  var PDF_BASES = [
    'assets/vendor/pdfjs/',
  ...
  /** A vendored copy is only usable if the server really has it. */
  ...
  /** Office text lives in runs inside paragraphs; keep the paragraph breaks. */
  ...
  /** Speaker notes are reached through the slide's relationship file, not by number. */
  ...
  async function readPptx(file, emit) {
    var zip = root.AR.zip.open(await file.arrayBuffer());
  ...
  /**
   * Documents have no slide or page breaks to lean on, so they are cut into
   * roughly slide-sized pieces, preferring to break where a heading starts.
   */
  ...
  /**
   * Reads every file in turn and returns the whole material as units.
   * onProgress is called per slide or page so the page can show movement.
   */
  ...
  root.AR = root.AR || {};
  root.AR.extract = { read: read, limits: { units: MAX_UNITS, unitChars: MAX_UNIT_CHARS } };
